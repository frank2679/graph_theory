...
\begin{minipage}{.5\textwidth}
  \centering
  \includegraphics[width=.8\linewidth]{./figure/5_11.png}
  \caption{Greedy ear decomposition}
  \label{11}
\end{minipage}%
\begin{minipage}{.5\textwidth}
  \centering
  \includegraphics[width=.8\linewidth]{./figure/5_14.png}
  \caption{$Q_4$}
  \label{14}
\end{minipage}
\end{figure}
...
=======
>>>>>>> ce9f6862c61c1b636c2d4ae0ab1a535a0fee5675
\end{proof}
...
\item[(2)]
\begin{proof}
Graph $G$ is minimally $k$-edge-connected. So $\delta(G) \leq k$. Then we find the vertex whose degree is $k$.\\
We can find a pair vertice $x,y$ with $k$ edge-disjoint paths between $x,y$. Add vertice $x^\prime, y^\prime$ connected with $x,y$ respectively. Then draw a corresponding graph $G^\prime$ of $G$ with vertex as edge of $G$ and edge existing if edges in $G$ are connected. \\
In this way, $G^\prime$ will be a minimally $k$-connected graph. According to \textbf{5.17(1)}, $\delta(G^\prime) = k$. 
We claim $\delta_{G^\prime}(xx^\prime)=k$. Because if another pair $u,v$ which has more than $k$ edge-disjoint paths, $\delta_{G^\prime}(u) = k+1$. 
Then in graph $G$, $deg(x) = k$, i.e. $\delta(G) = k$.    
\end{proof}
